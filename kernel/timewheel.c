        ...
        AIXOS_LIST_FOR_EACH_SAFE(position, next, &tw_level2.slots[high_slot]) {
            ...
            if ((int32_t)(now - tcb->wake_tick) >= 0) {
                ...
                if (remaining < AIXOS_CFG_TW_SIZE1) {
                    uint32_t new_slot =
                        (tw_level1.current_tick_low + remaining - 1U) %
                        AIXOS_CFG_TW_SIZE1;
                    aixos_list_add_tail(&tcb->timeout_node,
                                        &tw_level1.slots[new_slot]);
                } else {
                    // 仍在第二层，重新插入
                    uint32_t new_slot =
                        ((remaining - AIXOS_CFG_TW_SIZE1) >>
                         AIXOS_CFG_TW_BITS1) % AIXOS_CFG_TW_SIZE2;
                    new_slot = (tw_level2.current_tick_high + new_slot) %
                               AIXOS_CFG_TW_SIZE2;
                    aixos_list_add_tail(&tcb->timeout_node,
                                        &tw_level2.slots[new_slot]);
                }
            }
        }
